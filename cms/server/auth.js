...
var restfulService = require("./restService");
var bcrypt = require('bcrypt');
const jwa = require('jwa');
const hmac = jwa('hs256');

function extendDefaultConfig(defaultConfig, configToSet){
...
function tokenSignFromDb(user, db, config, fn){
	if(!user){
		console.log("userid or password error");
		fn("");
		return;
	};
//	console.log(user);
...
//curl http://localhost:8088/api/user?access_token=token1
//curl --data "userid=user2&password=123457" http://localhost:8088/api/authenticate
	var db = rest.db;
